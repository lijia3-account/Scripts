...
def load_and_preprocess(train_path, external_path):
    ...
    # 在 return 语句前添加：
    group_train = train_df.loc[train_mask, group_col].values
    group_val = train_df.loc[val_mask, group_col].values
    group_test = ext_df[group_col_ext].values
...
def main():
    ...
    print("加载数据...")
    (X_train, y_train, X_val, y_val, X_test, y_test,
    train_ids, val_ids, test_ids, feature_names, positive_label,
    group_train, group_val, group_test) = load_and_preprocess(
    ...
    for name, model in models.items():
        ...
        # 保存内部验证集结果
        # 修正：Group 列使用 group_val 而非 group_train
        df_val = pd.DataFrame({
            'Sample_ID': val_ids,
            'Group': group_val,
        ...
        # 保存外部验证集结果
        # 修正：Group 列使用 group_test 而非 group_train
    ...
    plt.title(f'ROC Curves - External Validation Set ({disease_type})', fontsize=20)
